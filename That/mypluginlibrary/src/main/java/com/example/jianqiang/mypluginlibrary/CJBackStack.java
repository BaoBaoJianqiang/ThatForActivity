...
public class CJBackStack {
    ...
    public IRemoteActivity launch(IRemoteActivity pluginAty) {
        if (atyStack == null) {
            atyStack = new LinkedList<IRemoteActivity>();
            add(pluginAty);
            return null;
        } else {
    ...
    private void add(IRemoteActivity pluginAty) {
     ...
     */
    private IRemoteActivity query(IRemoteActivity pluginAty) {
        atyStack.add(pluginAty);
        IRemoteActivity inStackPluginAty = null;
        for (int i = atyStack.size() - 2; i >= 0; i--) {
            if (atyStack.get(i).getClass().getName().equals(pluginAty.getClass().getName())) {
                switch (pluginAty.getLaunchMode()) {
                    case LaunchMode.STANDARD:// 什么也不做
                        break;
                    case LaunchMode.SINGLETOP:// 栈顶唯一
